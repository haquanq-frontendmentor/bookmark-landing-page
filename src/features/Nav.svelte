<script lang="ts">
  ...
  const mobileMenuButtonAttachment: Attachment<HTMLButtonElement> = (button) => {
    button.addEventListener("click", (e) => {
      const expanded = button.getAttribute("aria-expanded") === "true";
      button.setAttribute("aria-expanded", String(!expanded));
      menuOpening = !expanded;
    });

    window.addEventListener("resize", () => {
      if (window.matchMedia("(min-width: 48em)").matches && menuOpening) {
        menuOpening = false;
      }
    });
  };
  ...
  $effect(() => {
    document.body.style.overflow = menuOpening ? "hidden" : "";
  });
</script>
...
<nav>
  <Container>
    <div class="flex items-center justify-between py-12 md:pl-[0.375rem]">
        ...
        <svg width="148" height="25" xmlns="http://www.w3.org/2000/svg">
          <g fill="none" fill-rule="evenodd">
            <path
              ...
              fill={menuOpening ? "var(--color-white)" : "#242A45"}
              fill-rule="nonzero"
            />
            <g>
              <circle
                class="transition-colors"
                fill={menuOpening ? "var(--color-white)" : "#5267DF"}
                cx="12.5"
                cy="12.5"
                r="12.5"
              />
              <path
                ...
                fill={menuOpening ? "var(--color-blue-950)" : "var(--color-white"}
              />
            </g>
          </g>
        </svg>
      ...
      <button
        class={cn(
          "relative z-100 flex h-6 flex-col items-center justify-center *:h-[0.1875rem] *:w-6 *:shrink-0 *:bg-blue-950 *:transition-all *:duration-300 md:hidden",
          menuOpening && "*:bg-white",
        )}
        type="button"
        aria-label="Mobile menu"
        aria-expanded="false"
        aria-controls="mobile-menu"
        {@attach mobileMenuButtonAttachment}
      >
        <span class={cn(menuOpening && "rotate-45")}></span>
        <span class={cn("my-1", menuOpening && "hidden")}></span>
        <span class={cn(menuOpening && "-mt-[0.1875rem] -rotate-45")}></span>
      </button>
      <div
        class={cn(
          "absolute inset-0 z-50 hidden bg-[hsla(229,31%,21%,95%)] px-8 pt-[6.5625rem] md:static md:block md:bg-transparent md:p-0",
          menuOpening && "block",
        )}
        id="mobile-menu"
      >
